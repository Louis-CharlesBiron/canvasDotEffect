...
class CanvasUtils {
    ...
    static drawDotConnections(dot, color=[255,255,255,1], isSourceOver=false) {
        ...
        if (dc_ll) for (let i=0;i<dc_ll;i++) {
            ...
            ctx.strokeStyle = formatColor(color) // TODO COLOR
            ctx.beginPath()
    ...
    static drawOuterRing(dot, color=[255,255,255,1], radiusMultiplier) {
        let ctx = dot.ctx
        ctx.strokeStyle = formatColor(color) // TODO COLOR
        ctx.beginPath()
    ...
    static drawConnections(dot, color=[255,255,255,1], sourcePos) {
        let ctx = dot.ctx
        ctx.strokeStyle = formatColor(color) // TODO COLOR
        ctx.beginPath()
